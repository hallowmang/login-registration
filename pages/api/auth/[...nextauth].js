  ...
  providers:[
    ...
    CredentialsProvider({
      ...
      async authorize(credentials, req){
        ...
        let result;
        await fetch(`http://localhost:3000/api/auth/signup?password=${credentials.password}&id=${credentials.id}`,{headers: { 'Content-Type': 'application/json' }})
              .then(res=>res.json())
        ...
        //incorrect password
        if(result[0].password !== credentials.password || result[0].id !== credentials.id ){
